...
class Parser {    
    ...
    */
    ...
    async fetchWebPages() {
        ...
        console.log("pagesToFetch", pagesToFetch.length);
        ...
        console.log("calling addParsersToPages");
        await this.addParsersToPages(pagesToFetch);
        console.log("finished calling addParsersToPages");
        let index = 0;
        try
        {
            ...
            while (0 < group.length) {
                ...
                console.log("fetchingWebPageContent");
                ...
                console.log("group2", group.length);
            }
        }
        catch (err)
        {
            console.log("err instead of grouping")
            ErrorLog.log(err);
        }
    }
    ...
    async fetchWebPageContent(webPage) {
        ...
        return pageParser.fetchChapter(webPage.sourceUrl).then(function (webPageDom) {
            ...
            console.log("pageParser.preprocessRawDom(webPageDom);")
            ...
            console.log("return pageParser.fetchImagesUsedInDocument(content, webPage);")
    ...
    */
    ...
    getRateLimit()
    {
        let manualDelay = this.userPreferences.manualDelayPerChapter.value;
        console.log("getting manual delay " + manualDelay)
        if (manualDelay === "simulate_reading") {
            return manualDelay;
        }
        let manualDelayPerChapterValue = parseInt(manualDelay);

        let override = this.userPreferences.overrideMinimumDelay.value
        if (!override) {
