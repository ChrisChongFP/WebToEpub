...
class ChapterUrlsUI {
    ...
    populateChapterUrlsTable(chapters) {
        ...
        let skipUntil = "";
        if (params.has("skipUntilChapter")) {
            skipUntil = decodeURIComponent(params.get("skipUntilChapter"));
            skipUntil = skipUntil.toLowerCase().trim();
            console.log("skipUntilChapter: " + skipUntil);
        }
        let found = false;
        chapters.forEach(function (chapter) {
            if (skipUntil !== "" && !found) {
                ...
                console.log("found skip until" + chapter.sourceUrl);
                found = true;
                return;
            }
    ...
    static limitNumOfChapterS(maxChapters) {
        console.log("maxChapters start: " + maxChapters);
        let max = util.isNullOrEmpty(maxChapters) ? 10000 : parseInt(maxChapters.replace(",", ""));
        console.log("maxChapters after casting: " + max);
